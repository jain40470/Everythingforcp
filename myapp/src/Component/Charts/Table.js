...
const problemstried = (submissions) => {
    ...
    return [...new Set(submissions.data.result.map((p) => p.problem.name))].length;
}

const problemsolved = (submissions) => {
    return submissions.data.result.filter((problems) => problems.verdict === "OK").length;
}

const maxattempts = (submissions) => {
    var solved = submissions.data.result.filter((problems) => problems.verdict === "OK");
    var counts = {};
    var maxele = 1;
...
export default function Table(props) {
    ...
    return (
        <div className='flex-col place-content-center w-3/5'>
            {/* problems table */}
            <div>
                ...
                <table className='my-12 border-2 border-black'>
                    <tbody>
                        ...
                        <tr className='hover:bg-slate-400'>
                            ...
                            <td className='px-5 py-3' style={CurrentRating()}>{props.info.data.result[0].rating}</td>
                        </tr>
                        <tr className='hover:bg-slate-400'>
                            <td className='px-5 py-3'>Best Rank</td>
                            <td className='px-5 py-3' style={BestRating()}>{props.info.data.result[0].maxRating}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
